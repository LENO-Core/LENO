...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::LENO, nValue);
...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColuleno)
{
    std::string Table = table;
    for (int i = 0; i < nRows; i++)
        Table += makeHTMLTableRow(pCells + i * nColuleno, nColuleno);
    Table += "</table>";
    return Table;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (lenocore.conf).");
            QMessageBox::warning(this, "LenoCore Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
