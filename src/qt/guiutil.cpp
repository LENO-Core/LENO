...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a LenoCore address (e.g. %1)").arg("C6M1kT6PQaieDqNv2ajcTJtcfip2krta6a"));
...
{
    // return if URI is not valid or is no LenoCore: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::LENO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert lenocore:// to lenocore:
    //
    //    Cannot handle this later, because lenocore:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LENO, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open lenocore.conf with the associated application */
...
int TableViewLastColumnResizingFixer::getColulenoWidth()
{
    int nColulenoWidthSum = 0;
    for (int i = 0; i < columnCount; i++) {
        nColulenoWidthSum += tableView->horizontalHeader()->sectionSize(i);
    }
    return nColulenoWidthSum;
}
...
int TableViewLastColumnResizingFixer::getAvailableWidthForColumn(int column)
{
    ...
    if (nTableWidth > 0) {
        int nOtherColsWidth = getColulenoWidth() - tableView->horizontalHeader()->sectionSize(column);
...
// Make sure we don't make the coluleno wider than the tables viewport width.
void TableViewLastColumnResizingFixer::adjustTableColulenoWidth()
{
    ...
    int nTableWidth = tableView->horizontalHeader()->width();
    int nColsWidth = getColulenoWidth();
...
{
    if ((getColulenoWidth() - this->tableView->horizontalHeader()->width()) != 0) {
        disconnectViewHeadersSignals();
        resizeColumn(secondToLastColumnIndex, getAvailableWidthForColumn(secondToLastColumnIndex));
        connectViewHeadersSignals();
    }
 ...
 * the resize modes of the last 2 coluleno of the table and
 */
TableViewLastColumnResizingFixer::TableViewLastColumnResizingFixer(QTableView* table, int lastColMinimumWidth, int allColsMinimumWidth) : tableView(table),
                                                                                                                                          ...
                                                                                                                                          allColulenoMinimumWidth(allColsMinimumWidth)
{
    columnCount = tableView->horizontalHeader()->count();
    lastColumnIndex = columnCount - 1;
    secondToLastColumnIndex = columnCount - 2;
    tableView->horizontalHeader()->setMinimumSectionSize(allColulenoMinimumWidth);
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LenoCore.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for LenoCore.lnk
...
{
    return GetAutostartDir() / "lenocore.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a lenocore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LenoCore\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lenocore app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add lenocore app to startup item list
