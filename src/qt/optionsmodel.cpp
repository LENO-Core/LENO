...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LENO);
    ...
    if (!settings.contains("nAnonymizeLenoCoreAmount"))
        settings.setValue("nAnonymizeLenoCoreAmount", 1000);

    nAnonymizeLenoCoreAmount = settings.value("nAnonymizeLenoCoreAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLenoCoreAmount"))
        SoftSetArg("-anonymizelenocoreamount", settings.value("nAnonymizeLenoCoreAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lenocore.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLenoCoreAmount:
            return QVariant(nAnonymizeLenoCoreAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLenoCoreAmount:
            nAnonymizeLenoCoreAmount = value.toInt();
            settings.setValue("nAnonymizeLenoCoreAmount", nAnonymizeLenoCoreAmount);
            emit anonymizeLenoCoreAmountChanged(nAnonymizeLenoCoreAmount);
            break;
        case CoinControlFeatures:
