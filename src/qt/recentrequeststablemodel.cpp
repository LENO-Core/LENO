...
RecentRequestsTableModel::RecentRequestsTableModel(CWallet* wallet, WalletModel* parent) : walletModel(parent)
{
    ...
    /* These coluleno must match the indices in the ColumnIndex enumeration */
    coluleno << tr("Date") << tr("Label") << tr("Address") << tr("Message") << getAmountTitle();
...
QVariant RecentRequestsTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Horizontal) {
        if (role == Qt::DisplayRole && section < coluleno.size()) {
...
{
    coluleno[Amount] = getAmountTitle();
...
void RecentRequestsTableModel::sort(int column, Qt::SortOrder order)
{
    ...
    emit dataChanged(index(0, 0, QModelIndex()), index(list.size() - 1, NUMBER_OF_COLULENO - 1, QModelIndex()));
}
