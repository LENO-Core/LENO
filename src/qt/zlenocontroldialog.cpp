...
#include "ui_zlenocontroldialog.h"
...
std::set<std::string> ZLENOControlDialog::setSelectedMints;
std::set<CMintMeta> ZLENOControlDialog::setMints;

ZLENOControlDialog::ZLENOControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZLENOControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZLENOControlDialog::~ZLENOControlDialog()
{
    delete ui;
}

void ZLENOControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLENOControlDialog::updateList()
{
...
void ZLENOControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLENOControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLENO_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLENOControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLENOControlDialog::GetSelectedMints()
{
...
void ZLENOControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
