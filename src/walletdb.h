...
public:
    ...
    bool WriteZLENOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLENOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLENOSeed_deprecated(uint256& seed);
    bool EraseZLENOSeed();
    bool EraseZLENOSeed_deprecated();

    bool WriteZLENOCount(const uint32_t& nCount);
    bool ReadZLENOCount(uint32_t& nCount);
