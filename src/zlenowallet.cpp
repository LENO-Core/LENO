...
CzlenoWallet::CzlenoWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zleno seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLENOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLENOSeed_deprecated()) {
                    LogPrintf("%s: Updated zleno seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zleno seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zleno wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzlenoWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLENOCount(nCountLastUsed);
    else if (!walletdb.ReadZLENOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzlenoWallet::Lock()
{
    seedMaster = 0;
}

void CzlenoWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzlenoWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLENO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzlenoWallet::LoadMintPoolFromDB()
{
...
void CzlenoWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzlenoWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzlenoWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlenoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzlenoWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLENO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlenoTracker which also adds to database
    pwalletMain->zlenoTracker->Add(dMint, true);
...
void CzlenoWallet::SeedToZLENO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzlenoWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzlenoWallet::GenerateDeterministicZLENO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzlenoWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzlenoWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
