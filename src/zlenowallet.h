...
#ifndef LenoCore_ZLENOWALLET_H
#define LenoCore_ZLENOWALLET_H
...
public:
    CzlenoWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZLENO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZLENO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //LenoCore_ZLENOWALLET_H
