...
#if defined(NDEBUG)
#error "LenoCore cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uLENO) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zleno spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zleno is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zleno spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zleno without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zleno spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zleno spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zleno");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zleno mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 222 && nHeight > Params().LAST_POW_BLOCK()) {
		nSubsidy = 1.5 * COIN;
	}
	else if (nHeight <= 10000 && nHeight > 222) {
		nSubsidy = 1.6 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZLENOStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZLENOStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZLENOStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from LenoCore
...
{
	RenameThread("lenocore-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZLENOMinted()
{
...
void RecalculateZLENOSpent()
{
	...
	while (true) {
		...
		//Remove spends from zleno supply
...
bool RecalculateLENOSupply(int nHeightStart)
{
...
{
	// LenoCore: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZLENOSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZLENOMinted();
		RecalculateZLENOSpent();
		RecalculateLENOSupply(Params().Zerocoin_StartHeight());
	}

	//Track zleno money supply in the block index
	if (!UpdateZLENOSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zleno supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zLENOSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zleno spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zleno serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zleno stake block is less than allowed start height", __func__);

	if (CZLENOStake* zleno = dynamic_cast<CZLENOStake*>(stake)) {
		CBlockIndex* pindexFrom = zleno->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zleno stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zleno stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zleno->GetValue()));
		if (nChecksum200 != zleno->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zleno->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZLENO() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zleno fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> LENOInputs;
		std::vector<CTxIn> zlenoInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zlenoInputs.push_back(stakeIn);
			}
			else {
				LENOInputs.push_back(stakeIn);
			}
		}
		const bool hasLENOInputs = !LENOInputs.empty();
		const bool hasZLENOInputs = !zlenoInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasLENOInputs)
					...
					for (CTxIn LENOIn : LENOInputs) {
						if (LENOIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zleno inputs.
			if (hasZLENOInputs) {
				for (CTxIn zLENOInput : zlenoInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zLENOInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zleno");
...
/** Compute what height to leno back to with the CBlockIndex::pskip pointer. */
int static inline GetSkipHeight(int height)
{
	if (height < 2)
		return 0;

	// Determine which height to leno back to. Any number strictly lower than height is acceptable,
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zleno mints and %d zleno spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// LenoCore: We use certain sporks during IBD, so check to see if they are
