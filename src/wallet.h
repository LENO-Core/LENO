...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 LENO at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zleno send
enum ZerocoinSpendStatus {
    ZLENO_SPEND_OKAY = 0,                            // No error
    ZLENO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLENO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLENO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLENO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLENO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLENO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLENO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLENO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLENO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLENO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLENO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLENO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLENO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLENO_SPENT_USED_ZLENO = 14,                      // Coin has already been spend
    ZLENO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLENO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZLENOBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZLENOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzlenoAuto) const;
    ...
    */
    ...
    CzlenoWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzlenoTracker> zlenoTracker;
    ...
    void setZWallet(CzlenoWallet* zwallet)
    {
        zwalletMain = zwallet;
        zlenoTracker = std::unique_ptr<CzlenoTracker>(new CzlenoTracker(strWalletFile));
    }

    CzlenoWallet* getZWallet() { return zwalletMain; }
    ...
    /** zleno reset */
    boost::signals2::signal<void()> NotifyzlenoReset;
