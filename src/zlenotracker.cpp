...
CzlenoTracker::CzlenoTracker(std::string strWalletFile)
{
...
CzlenoTracker::~CzlenoTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzlenoTracker::Init()
{
...
bool CzlenoTracker::Archive(CMintMeta& meta)
{
...
bool CzlenoTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzlenoTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzlenoTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzlenoTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzlenoTracker::GetSerialHashes()
{
...
CAmount CzlenoTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzlenoTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzlenoTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzlenoTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzlenoTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzlenoTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzlenoTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzlenoTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzlenoTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzlenoTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzlenoTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzlenoTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzlenoTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzlenoTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzlenoTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzlenoTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzlenoTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzleno from DB\n", __func__, listDeterministicDB.size());
    }
